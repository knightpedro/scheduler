...
const JobsPerformanceContainer = props => {
  ...
  const jobsInProgress = useMemo(() => jobs.filter(j => !j.isComplete).length, [
    jobs
  ]);
  ...
  const minYear = useMemo(() => {
    ...
    const earliestRecord = jobs.reduce((acc, current) => {
      return current.dateReceived.isBefore(acc) ? current.dateReceived : acc;
    }, moment());
  ...
  const currentMonthJobsReceivedCount = useMemo(() => {
    ...
    return jobs.filter(j => j.dateReceived.isSameOrAfter(startOfMonth)).length;
  }, [jobs]);
  ...
  const coordinatorWorkLoadChartData = useMemo(() => {
    ...
    return coordinators.map(coordinator => ({
      label: coordinator.name
        ...
        .map((n, i) => (i === 0 ? n[0] : n))
        .join(" "),
      value: selectedJobs.filter(j => j.coordinatorId === coordinator.id).length
    }));
  ...
  const jobsReceivedChartData = useMemo(() => {
    ...
    return intervals.map(interval => ({
      ...
      value: jobs.filter(j => j.dateReceived.isSame(interval, period)).length
    }));
  }, [jobs, period, minYear]);
    ...
    axios
      ...
      .then(
        axios.spread((jobsRes, coordinatorsRes) => {
          ...
          setCoordinators(coordinatorsRes.data.coordinators.sort(sortByName));
        })
      )
    ...
    <>
      ...
      <Row className="align-items-center">
        <Col lg>
          ...
          <CoordinatorWorkLoadChart data={coordinatorWorkLoadChartData} />
        </Col>
        <Col lg>
          <h4 className="text-center mt-5">Jobs Received</h4>
          <JobsReceivedChart data={jobsReceivedChartData} xLabel={period} />
        </Col>
      </Row>
      <Row className="align-items-center">
        <Col lg={5}>
