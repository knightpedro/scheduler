...
class EditResourceFormContainer extends React.Component {
    ...
    handleSubmit = (values, { setSubmitting }) => {
        ...
        if (isEqual(values, resource)) {
            ...
            axios.put(`${RESOURCES_URL}/${resource.id}`, values)
            .then(() => {
                const resourceDetailPath = generatePath(Routes.resources.DETAIL, { id: resource.id });
    ...
    renderBreadcrumb(resource) {
        const resourcePath = generatePath(Routes.resources.DETAIL, { id: resource.id });
        return (
            <Breadcrumb>
                <Breadcrumb.Item href={Routes.resources.LIST}>Plant</Breadcrumb.Item>
                <Breadcrumb.Item href={resourcePath}>{resource.name}</Breadcrumb.Item>
    ...
    render() {
        ...
        return (
            <Container>
                ...
                <EditResourceForm resource={resource} handleSubmit={this.handleSubmit} handleCancel={this.handleCancel}/>
            </Container>
        );
    }
}
